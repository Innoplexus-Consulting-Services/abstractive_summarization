...
def _file_vocab(args):
    filenamepath = args
    document = _get_json_file_data(filenamepath)
    body = document['body']
    ...
    #words_body = words_body.extend(words_abstract)
    return words_body + words_abstract

    #use words_body to update the dict of word
...
def _text_to_vocabulary(input_folder, output_file):
    ...
    total = len(file_list)
    count_dict = mp.Manager().dict()
    start = time()
    p = mp.Pool()
    count = 0
    result = p.map(_file_vocab, file_list)
    print(len(result))
    for l in result:
        count += 1
        if count % 10 == 0:
    ...
    print('Counter length ' + str(sum(count_dict.values())))
    ...
    print('\nVocabulary file generated!! Time taken: ' + str(time() - start))
